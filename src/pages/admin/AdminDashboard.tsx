    ...
    <div className="min-h-screen flex bg-muted">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-hero text-hero-foreground flex flex-col shrink-0 transform transition-transform lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 border-b border-hero-foreground/10 flex items-center justify-between">
          ...
          <button className="lg:hidden text-hero-foreground/70" onClick={() => setSidebarOpen(false)}>
            <X className="h-5 w-5" />
          </button>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted">
                <tr>
                  ...
                  <th className="text-left p-3 font-medium whitespace-nowrap">Visa Status</th>
              ...
              <tbody>
                {apps.slice(0, 5).map(app => (
                  <tr key={app.id} className="border-t">
                    ...
                    <td className="p-3 whitespace-nowrap">{app.visaStatus || "—"}</td>
...
function ApplicationsTab() {
  ...
  const filteredApps = apps.filter(app => {
    ...
    return app.fullName.toLowerCase().includes(s) ||
      app.email.toLowerCase().includes(s) ||
      app.jobTitle.toLowerCase().includes(s) ||
      app.nationality.toLowerCase().includes(s) ||
      app.visaStatus.toLowerCase().includes(s) ||
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {selected ? (
        ...
        <p className="text-muted-foreground">{search ? "No applications match your search." : "No applications received yet."}</p>
      ) : (
        <div className="bg-card rounded-lg border overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted">
              <tr>
                ...
                <th className="text-left p-3 font-medium whitespace-nowrap">Visa</th>
                ...
                <tr key={app.id} className="border-t hover:bg-muted/50">
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => handleDelete(app.id)} className="text-destructive hover:text-destructive">
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-card rounded-lg border p-4 sm:p-6 space-y-4 max-w-lg">
        <div>
          ...
          <Input id="botToken" value={settings.botToken} onChange={e => setSettings(s => ({ ...s, botToken: e.target.value }))} placeholder="123456789:ABCdefGHIjklMNOpqrsTUVwxyz" type="password" />
          <p className="text-xs text-muted-foreground mt-1">Get this from @BotFather on Telegram</p>
        </div>
        <div>
          <Label htmlFor="chatId">Chat ID</Label>
          <Input id="chatId" value={settings.chatId} onChange={e => setSettings(s => ({ ...s, chatId: e.target.value }))} placeholder="-1001234567890" />
...
function SEOTab() {
  ...
  const addKeyword = () => {
    ...
    if (!kw) return;
    if (settings.searchKeywords.includes(kw)) {
  ...
  const removeKeyword = (kw: string) => {
    setSettings(s => ({ ...s, searchKeywords: s.searchKeywords.filter(k => k !== kw) }));
  };

  return (
    <div className="space-y-6 animate-fade-in">
      <h1 className="font-heading text-2xl font-bold">SEO & Search Console</h1>
      <p className="text-muted-foreground text-sm">Manage Google Search Console verification and target keywords.</p>
      ...
      <div className="bg-card rounded-lg border p-4 sm:p-6 space-y-6 max-w-2xl">
        <div>
          ...
          <Input
            ...
            onChange={e => setSettings(s => ({ ...s, searchConsoleId: e.target.value }))}
      ...
      <div className="bg-secondary rounded-lg p-4 sm:p-6 max-w-2xl">
        <h3 className="font-heading font-semibold mb-2">Built-in SEO Features</h3>
        <ul className="text-sm text-muted-foreground space-y-1 list-disc pl-5">
          <li>Auto-generated sitemap.xml for Google crawling</li>
          <li>Structured data (JSON-LD) on every page</li>
          <li>Optimised meta tags for all pages</li>
          <li>Robots.txt configured for major search engines</li>
